...
KNN Imputation preserves the original distribution **most effectively**, capturing local patterns and producing the most realistic fill-ins for missing ages.


:::

## Imputation Comparisons {.smaller}
...
Mean imputation causes a sharp artificial peak around 28–30 years. It distorts the original distribution and reduces variability.
...
# Age plot after regression imputation
ggplot(train_regression, aes(x = Age)) +
  geom_histogram(fill = "green", alpha = 0.6, bins = 30) +
  labs(title = "Age Distribution After Regression Imputation", x = "Age", y = "Count")

```


Regression imputation maintains a more realistic distribution and spreads values more naturally than mean imputation.
...
ggplot(train_knn, aes(x = Age)) +
  geom_histogram(fill = "gold", alpha = 0.5, bins = 30) +
  labs(title = "Age Distribution After KNN Imputation", x = "Age", y = "Count")
```
<small>
